...
# btn1 = ttk.Button(root, text="Calendar", command=cal_func)
# btn2 = ttk.Button(root, text="DateEntry", command=date_func)
#
# btn1.pack()
# btn2.grid()
# clock function
...
class FileZap(tk.Frame):
    def __init__(self, win):
        ...
        for index in range(len(add_acc_lbls)):
            self.add_acc_menu.add_command(label=add_acc_lbls[index])
        ...
        self.exit_menu.add_command(label="Exit", command=exit_gui)
...
detail_label = ttk.Label(text="Invoice No.")
...
stock_label = ttk.Label(text="Current Stock")
...
# date_from = unposted_cal_value
txt.insert(0.0, str(unposted_cal_value()))
...
posted_btn = ttk.Button(root, text="Posted", command=cal_func)
...
unposted_btn = ttk.Button(root, text="UnPosted", command=cal_func)
